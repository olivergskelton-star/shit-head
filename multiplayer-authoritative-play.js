...
(() => {
  ...
  function attachHostActionListener(conn) {
    ...
    conn.on('data', (data) => {
      ...
      // Execute the move on the HOST, using the same engine as local play.
      state.selected = indices;
      playSelected(player);

      // render() already schedules a host publish; publish immediately as well so
      // the client never sits waiting on a delayed state echo.
