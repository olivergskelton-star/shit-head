...
test('real drink artwork stays visible in picker and after drink re-render', async ({ browser }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return mats.length === 3 && mats.every((mat) => {
      const coaster = mat.querySelector('.beer-mat-coaster-asset .asset-sprite-sheet');
      const drink = mat.querySelector('.beer-mat-drink-asset .asset-sprite-sheet');
  ...
  await page.waitForFunction(() => [...document.querySelectorAll('.drink-picker-option')].every((option) => {
    const image = option.querySelector('.asset-sprite-sheet');
    return image?.complete && image.naturalWidth > 0;
  }));

  const guinness = page.locator('.drink-picker-option[data-drink="guinness"]');
  await expect(guinness.locator('.asset-sprite-sheet')).toBeVisible();
  ...
  await expect(page.locator('.self-beer-mat .asset-sprite-fallback')).toBeHidden();
  await context.close();
});
